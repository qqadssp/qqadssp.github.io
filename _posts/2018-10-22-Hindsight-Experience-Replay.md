...
$$
Q^*(s,a)=E_{s'~p(\cdot | s,a)}[r(s,a)+\gamma \max Q^*(s',a')]
$$
...
　　每个计算步agent不仅使用当前状态，还使用当前目标$\pi:S \times G \rightarrow A$作为输入，得到奖励$r_t=r_g(s_t,a_t)$。Q函数现在不仅依赖于状态-动作对，而且依赖于目标$Q^\pi(s_t,a_t,g)=E[R_t|s_t,a_t,g]$。Schaul等人(2015a)展示在这种设置下，使用直接自Bellman方程的导出(就如同DQN)训练一个Q函数近似器是可能的，从此导出的贪婪策略可以扩展到先前未见到的状态-动作对。这个对DDPG扩展的方法是直接了当的。  
