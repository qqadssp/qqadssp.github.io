...
　　考虑一个agent，看到一个观察$x_t$，做出一个动作$a_t$然后转到下一个观察为$x_{t+1}$的状态。我们想要以关于这个转换有多少丰富性的奖励$r_t$激励这个agent。为了提供这个奖励，我们使用包含一下因素的探索奖励：(a)一个将观察嵌入到表征$\phi(x)$的网络，(b)一个前馈网络，在前一个观察和动作$p(\phi(x_{t+1}) \mid x_t, a_t)$的条件下预测下一个状态的表征。给定一个转换元祖$\leftbrace x_t, x_{t+1}, a_t \rightbrace$，探索奖励被定义为$r_t=-\log p(\phi(x_{t+1}) \mid x_t, a_t)$，也叫作‘惊喜’。  
